...
export async function copyLocalTemplate(
  ...
  // Structure: VibeDraft/lib/download.js -> ../../VibeDraft/
  const specKitBaseDir = path.resolve(__dirname, '..', '..', 'VibeDraft');

  if (!(await fs.pathExists(specKitBaseDir))) {
    throw new Error(`Local VibeDraft directory not found: ${specKitBaseDir}`);
  }
  ...
  try {
    ...
    const scriptsSource = path.join(specKitBaseDir, 'scripts');
    ...
    const templatesSource = path.join(specKitBaseDir, 'templates');
    ...
    const memorySource = path.join(specKitBaseDir, 'memory');
    ...
    for (const file of rootFiles) {
      const sourceFile = path.join(specKitBaseDir, file);
...
export async function downloadAndExtractTemplate(
  ...
  // Use local templates if flag is set
  if (useLocalTemplate) {
