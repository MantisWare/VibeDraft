...
export async function copyLocalTemplate(
  ...
  try {
    ...
    // Copy root files
    // README.md -> VIBEDRAFT_README.md (preserve existing README)
    const readmeSource = path.join(vibeDraftBaseDir, 'README.md');
    if (await fs.pathExists(readmeSource)) {
      await fs.copy(readmeSource, path.join(projectPath, 'VIBEDRAFT_README.md'), { overwrite: true });
    }

    // .gitignore -> only copy if doesn't exist
    const gitignoreSource = path.join(vibeDraftBaseDir, '.gitignore');
    const gitignoreDest = path.join(projectPath, '.gitignore');
    if (await fs.pathExists(gitignoreSource) && !(await fs.pathExists(gitignoreDest))) {
      await fs.copy(gitignoreSource, gitignoreDest);
    }

    // spec-driven.md -> always copy
    const specDrivenSource = path.join(vibeDraftBaseDir, 'spec-driven.md');
    if (await fs.pathExists(specDrivenSource)) {
      await fs.copy(specDrivenSource, path.join(projectPath, 'spec-driven.md'), { overwrite: true });
    }
